...
class InvertedResidual(nn.Module): #MV2
    ...
    def __init__(
        ...
        out_channels: int,
        stride: int,
        expand_ratio: Union[int, float],  #第一个卷积层会把通道数翻几倍
        skip_connection: Optional[bool] = True, #是否要捷径分支
    ) -> None:
        assert stride in [1, 2]
        hidden_dim = make_divisible(int(round(in_channels * expand_ratio)), 8)  #1x1卷积（第一个后）把通道数调整成多少

        super().__init__()

        block = nn.Sequential()
        if expand_ratio != 1: #如果不为1，添加convlayerS
            block.add_module(
                name="exp_1x1",
                module=ConvLayer(
                    in_channels=in_channels,
...
class MobileViTBlock(nn.Module):
    ...
    def __init__(
        self,
        in_channels: int,
        transformer_dim: int, #输入encoder的toen序列从
        ffn_dim: int, #输入encoder第一个全连接层节点个数
        n_transformer_blocks: int = 2, #重复堆叠transformer次数
        head_dim: int = 32,
        attn_dropout: float = 0.0,
        dropout: float = 0.0,
        ffn_dropout: float = 0.0,
        patch_h: int = 8,#patch高宽
        patch_w: int = 8,
        conv_ksize: Optional[int] = 3, #nxn的那个conv
        attn_eff:bool=False,
        *args,
        **kwargs
    ) -> None:
        ...
        conv_1x1_out = ConvLayer(  #对应fusion部分卷积层
    ...
    def unfolding(self, x: Tensor) -> Tuple[Tensor, Dict]:
        patch_w, patch_h = self.patch_w, self.patch_h #传入patch宽和高
        patch_area = patch_w * patch_h
        batch_size, in_channels, orig_h, orig_w = x.shape #获取传入shape
        ...
        if new_w != orig_w or new_h != orig_h:
            ...
            x = F.interpolate(x, size=(new_h, new_w), mode="bilinear", align_corners=False)  #插值
            interpolate = True
        ...
        x = x.reshape(batch_size,in_channels,num_patches,patch_area)
        x = x.transpose(1,3)
        #patch_area对应一个patch(中方块)有多少像素（小方块），N对应每个序列token的个数
        x= x.reshape(batch_size*patch_area,num_patches,in_channels) 
...
class MobileViT(nn.Module):
    ...
    def _make_mit_layer(input_channel: int, cfg: Dict) -> Tuple[nn.Sequential, int]:
        ...
        block.append(MobileViTBlock(
            ...
            head_dim=head_dim,
            conv_ksize=3,
            attn_eff=cfg.get("attn_eff", False),
        ))
...
def mobile_vit_xx_small(num_classes: int = 2):
